...
系统提供了完善的用户会话管理功能，可以跟踪和管理用户在不同设备上的登录状态：

- 记录用户每次登录的设备信息、IP地址和登录时间
- 显示用户当前活跃的登录设备数量
- 允许管理员查看用户的所有活跃会话
- 支持强制登出用户的所有设备
...
管理员可以在用户管理界面中查看和管理用户的会话：

1. 在用户列表中可以看到每个用户当前活跃的设备数量
2. 编辑用户时可以点击"查看登录设备"按钮
3. 在设备列表中可以查看详细的设备信息、IP地址和登录时间
4. 可以一键登出用户的所有设备

#### 技术实现

- 使用JWT令牌进行身份验证
- 在数据库中存储会话信息，包括设备类型、IP地址和用户代理
- 自动清理过期的会话记录
- 支持多设备同时登录
